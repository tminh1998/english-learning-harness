...
nghĩa, và nó nối các buổi rời rạc thành một mạng từ vựng.

**Áp dụng thế nào:**
- Đánh dấu từ cũ: HTML `<span class="rev">…</span>`, Markdown `_từ cũ_`.
  **Không in đậm** — `<b>` / `**…**` chỉ dành cho từ mới của bài.
- Phải đúng nghĩa đã học, không phải trùng mặt chữ (`reach the client` không tính
  là ôn `reach out`).
- Ưu tiên từ đang tới hạn trong REVIEW_QUEUE và từ khác nhóm với từ mới.
