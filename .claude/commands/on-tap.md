...
1. `. tools/openit.sh; keove; hnay` — về nhánh chính, kéo bài từ máy khác về (R6),
   rồi lấy ngày thật. ⚠️ **Không dùng `date` trần**: trên VM cloud nó chạy giờ UTC,
   lấy sai ngày là lọc sai danh sách từ tới hạn.
...
7. `daylen "on-tap: $(hnay) — <đúng>/<tổng>"` (R6). Không mở branch, không tạo PR.
   Không in `DA-PUSH` → nói thẳng là kết quả ôn chưa lưu lên GitHub, đừng báo xong.
